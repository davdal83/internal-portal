...
async function loadPhotos(storeId) {
  ...
  if (error) {
    photosDiv.innerText = "Error loading photos.";
...
async function deletePhoto(photoId, imageUrl) {
  try {
    ...
    const filePath = url.pathname.substring(pathIndex + 1); // remove leading slash
...
document.getElementById("upload-form").addEventListener("submit", (e) => {
  ...
  if (!storeId || !file) {
    alert("Please select a store and an image file to upload.");
    return;
  }
